...
def gauss_jacobi(a, b, tolerance=1e-10, max_iterations=100):
    """
    Solves the system of linear equations Ax = b using Gauss-Jacobi method.
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(n):
            # Summation of a[i][j] * x[j] for j != i (using old x values only)
    ...
    raise Exception("Gauss-Jacobi method did not converge within the given iterations")
